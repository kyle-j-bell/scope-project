...
- Come up with at least two situations (one per person) on when currying would
  be useful
***********************************************************************/
//Declares a function that accepts a number as an argument and creates an empty array.
//Returns a function that accepts a single number as an argument when invoked and pushes it to the array.
//When the length of the array equals numArgs, the elements are added together using Array.reduce and the sum is returned.
//Otherwise, the function returns itself.
function curriedSum(numArgs) {
  let numbers = [];
  return function curriedAdder(num) {
    numbers.push(num);
